...
class Quiz:  # Принимаем все ответы пользователя
    ...
    def start_quiz(self):
        for question in self.quiz_data:
            ...
            self.user_answer = input(f'{self.question["question"]}\n').lower()
            self.print_answer()
            self.count_correct_answer()
        self.print_result_quiz()

    def check_answer(self) -> bool:  # Определяем правильный ли ответ
        return self.user_answer == self.question['correct_answer']

    def print_answer(self) -> None:  # Печать правильного/неправильного ответа, если неправильный - печать правильного
        self.correct_answer = self.check_answer()
    ...
    def count_correct_answer(self) -> None:  # Считает кол-во правильных ответов
        if self.correct_answer:
            self.score_correct += 1

    def print_result_quiz(self) -> None:  # Печать результата викторины
